...
const ManageOrders = () => {
    ...
    useEffect(() => {
        setIsloading(true);
        fetch(`https://sha-auto.herokuapp.com/orders`)
            .then(res => res.json())
            .then(data => {
                setBookings(data);
                setIsloading(false);
    ...
    const updateOrder = id => {
        axios.put(`https://sha-auto.herokuapp.com/orders/${id}`, {
            status: 'complete'
        })
            .then(res => {
                if (res.status === 200) {
        ...
        <div className='p-3'>
            <div className='m-3'>
                ...
                {isloading ? <div className='d-flex w-100 justify-content-center'>
                    ...
                    : <div>
                        {bookings.map(b => <div style={{ borderRadius: '10px', border: '1px solid orange' }} className='d-flex p-2 mb-3 align-items-center'>
                            <div className='w-100 d-flex align-items-center justify-content-around'>
                                <h6>User: {b.user_name}</h6>
                                ...
                                {b.status === 'pending' && <button onClick={() => updateOrder(b._id)} className='button'>Shiped</button>}
                            </div>
                        </div>)}</div>}
            </div>
        </div>
